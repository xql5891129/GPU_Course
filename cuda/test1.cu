...
__global__ void Convolution(half* input, half* output, half* weight, half* bias, 
                             int num_points, int in_channels, int out_channels) 
{
    // 定义共享内存
    ...
    // 使用共享内存存储权重和偏置
    half* shared_weights = sharedMem;  // 假设权重存储在共享内存的开始位置
    half* shared_bias = &sharedMem[in_channels * out_channels];  // 假设偏置存储在权重之后

    // 加载权重和偏置到共享内存
    if (threadIdx.x < in_channels * out_channels) {
    ...
    if (point_idx < num_points * out_channels) {
        ...
        // 初始化输出为偏置
        half value = shared_bias[row];
        for (int i = 0; i < in_channels; i++) {
            value = __hadd(value, __hmul(input[i * num_points + col], shared_weights[row * in_channels + i]));
        }
        output[point_idx] = value;
    }
}
